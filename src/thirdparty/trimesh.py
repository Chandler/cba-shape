...
class TriMesh(object):
    ...
    def update_halfedges(self):
        ...
        assert False not in [
            0 == len(out_heis) for out_heis in vertex2outgoing_boundary_hei.values()
        ]
    ...
    def FromOBJ_Lines(obj_lines):
        ...
        for line in obj_lines:
            ...
            if not sline:
                ...
                ## indices.
                face_vertex_ids = [
                    (ind - 1) if (ind >= 0) else (len(result.vs) + ind) for ind in face_vertex_ids
                ]
    ...
    def write_OBJ(self, fname, header_comment=None):
        ...
        if header_comment is None:
            ...
            header_comment = "Written by " + " ".join(
                [arg.replace("\n", r"\n") for arg in sys.argv]
            )
